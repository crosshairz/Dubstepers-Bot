...
mubBot.misc.Themes = "Dubstep, Remix's, House, and techno.   No Deathstep, rap, or reggae!";
mubBot.misc.help = "Type !rules for rules and !commands for commands" //What Help will say
mubBot.misc.version = "1.2.3";
...
mubBot.misc.changelog = "Since the last update of Plug.dj the bot is unable to see names or permissions so admin commands are temporarily disabled, also when the bot responds to your command it will refer to you as undefined becuase of the name visibility issue.Ther bot is alsounable to delete commands after they are said. We are working to fix these issues, please bear with us in the meantime.";
    ...
    API.on(API.CHAT, function(data){
        ...
        if(data.message.indexOf('!') === 0){
            ...
            if(mubBot.misc.ready || mubBot.admins.indexOf(fromID) > -1 || API.getUser(fromID).permission > 1){
                switch(command[0].toLowerCase()){
                    commented out because the bot isnt running on a dedicated bot account
                     case "meh":
                    ...
                    case "likeroom":
                        API.sendChat("If you like this room: refer your friends, add a bookmark/favorite to us in your browser, and come back often. :)");
                    break
                    ...
                    case "themes":
                    case "theme":
                        API.sendChat("The different themes allowed are: " + mubBot.misc.Themes);
                    break
